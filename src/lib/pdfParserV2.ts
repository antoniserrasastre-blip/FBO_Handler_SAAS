...
export async function parseCybermaxPdf(buffer: Buffer | Uint8Array): Promise<ParseResult> {
  ...
  try {
    ...
    // Each page is processed independently: same column anchors, same header
    // and footer filters apply per page. We deliberately do NOT concatenate
    // raw items across pages because each page's Y axis is independent and
    // grouping by Y would collide rows from different pages.
    for (let pageNum = 1; pageNum <= internal.doc.numPages; pageNum++) {
      const page = await internal.doc.getPage(pageNum);
      ...
      // Find the sheet date on page 1 only — every page shares the same date.
      if (pageNum === 1) {
